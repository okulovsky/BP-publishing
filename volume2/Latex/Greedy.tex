...
\node[ver, minimum size=0.7cm] (n0) at (0,0){\only<2>{0}\only<3->{\bf 0}};
\node[ver, minimum size=0.7cm] (n1) at (2,0) {\only<4-5>{6}\only<6-7>{5}\only<8>{4}\only<9>{\bf 4}};
\node[ver, minimum size=0.7cm] (n2) at (1,1) {\only<4>{1}\only<5->{\bf 1}};
\node[ver, minimum size=0.7cm] (n3) at (1,-1) {\only<4-6>{2}\only<7->{\bf 2}};
...
\node[ver, minimum size=0.7cm] (n0) at (0,0) {\only<2->{\bf 0}};
\node[ver, minimum size=0.7cm] (n1) at (2,0) {\only<5>{4}\only<6>{\bf 4}};
\node[ver, minimum size=0.7cm] (n2) at (1,1) {\only<3>{2}\only<4-7>{\bf 2}\only<8>{1}};
\node[ver, minimum size=0.7cm] (n3) at (1,-1) {\only<3-6>{5}\only<7->{\bf 5}};
...
\uncover<+->{Обозначим $V\setminus ${\tt notVisited} через $S$, ${\tt track}[v].{\tt Price}$ через $p(v)$, {\tt start} через $v_0$.}

\uncover<+->{\begin{theorem}
Пусть все веса в графе неотрицательны.  Тогда в момент раскрытия вершины $v$, $p(v)$ является длиной кратчайшего пути из $v_0$ в $v$.
\end{theorem}}
\uncover<+->{\begin{proof}
